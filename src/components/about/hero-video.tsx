...
>(({ children, className, ...props }, ref) => {
  ...
  if (isMobile) {
    return (
      <motion.div className={cn("relative", className)} ref={ref}>
        {children}
      </motion.div>
    )
  }
  ...
  return (
    <ContainerScrollContext.Provider value={{ scrollYProgress }}>
      <section
        className={cn(
          "relative w-full",
          isMobile ? "min-h-0 pb-0 pt-8" : "min-h-[120vh] pb-[30%] pt-8",
          className
        )}
        {...props}
        ref={scrollRef}
...
const ContainerInset = React.forwardRef<HTMLDivElement, ContainerInsetProps>(
  (
    ...
    const style = React.useMemo(
      () => isMobile ? { ...props.style } : ({ y, clipPath, ...props.style }),
      [isMobile, y, clipPath, props.style]
    )
    return (
      <motion.div
        transition={SPRING_CONFIG || props.transition}
